...
public class GroupRequestManager {
    ...
    public void acceptRequest(GroupRequest groupRequest) {
        ...
        if (!manager.isMember(groupRequest.getSender(), groupRequest.getGroup())) {
            ...
            groupRequest.getGroup().notifyAllMembers("A new member is added to a group you belong to");
            manager.save();
            manager.load();
        ...
        if (members != null) {
            ...
            for (Group g : manager.getGroups()) {
                if (!manager.isMember(user, group) && !isRequest(user, requests)) {
                    ...
                    Notifications.NotificationManager.getInstance().addNotification("You sent a request to "+group.getName(), user, null, "Group activity", false);
                    manager.save();
                }
            }
        }
